...
distanceCZ <- function(miles=100, output_path = "~/github/EconData/DataRepo/Miscellaneous/"){
  ...
  county_dist <- setDT(read.csv(file=sprintf("%scounty_fips_distance_%smiles.csv", output_path, miles)))
  ...
  county_dist <- county_dist[,list(state_fips1,county_fips1,state_fips2,county_fips2,distance = mi_to_county)][state_fips1 != "72" & state_fips2 != "72"]
  
  county_to_cz <- setDT(read.csv(file=sprintf("%scz_crosswalk_2000.csv",output_path)))
  setnames(county_to_cz, c("county_fips1","cz1","state_fips1"))
  county_dist <- merge(county_dist, county_to_cz, by=c("state_fips1","county_fips1"))
  setnames(county_to_cz, c("county_fips2","cz2","state_fips2"))
  ...
  county_dist <- unique(rbind(county_dist, county_dist[,list(cz1=cz2, cz2=cz1, distance)]))
...
distanceState <- function(miles=100, output_path = "~/github/EconData/DataRepo/Miscellaneous/"){
  ...
  county_dist[, state_fips2 := as.integer(substr(county2,1,2))]
  county_dist[, county_fips2 := as.integer(substr(county2,3,5))]
  county_dist <- county_dist[,list(state_fips1,state_fips2,distance = mi_to_county)][state_fips1 != "72" & state_fips2 != "72"]
